...
If you want to follow along you need to install Jupyter Notebook or Jupyter Lab as well as Kedro. All the examples are with the VS Code IDE. An ideal starting point is the end of my tutorial on [Setting up a Virtual Environment and VS Code for Kedro](/posts/quick_kedro_setup/) . It guides you through from start to having a blank, working Kedro project. The examples are from a Windows environment, so there may be subtle differences for Mac users. I have also prepared a branch with the output from this tutorial which you can clone using:
...
If you like, please download this {{% download "notebook" "/data/10_Minimal_Spaceship.ipynb" %}}  and the {{% download "data" "/data/train.csv" %}}. Feel free to explore, it is a pretty typical ML project. After loading the data, we do some pre-processing, figuring out if a passenger was travelling alone or in a group. Next, we take care of `NaN` values. Clearly there is more pre-processing we could do, but let's keep it simple.
...
So before creating a Kedro project and converting it, let's establish what the different parts of the pipeline would be. A good starting point is:
...
So the first step we need to do is to register the csv file in the catalog. Before we do that, let's discuss why you shouldn't reference files directly in the code. It works in the notebook, right? But let's imagine moving this project to the cloud, then the file reference will change as the file may be located somewhere else. Or the name changes. Or you are given a parquet file instead of a csv file. In all these cases you only need to edit the `catalog.yml` file and don't need to touch any code. This makes it easier to adapt and less likely to break. For the same reason user configurable parameters are saved in the `parameters.yml` file. Or logging details in `logging.yml`. I think you get the idea, it simply makes good sense to move anything that frequently change into a settings file so it can be modified without changing the code.
...
Let's load this now in our notebook. As we are in VS Code, we can open a new integrated terminal (right click in explorer), then use the `kedro jupyter lab` command to start the notebook editor:
...
First though we need to install the `pandas` and `sci-kit-learn` packages that will be used by the pipelines:
...
- doc_string (i.e comments) - annotations explaining the usage of the function. If you stick to this format, VS Code or other IDEs finds them and can display them as hints.
- return (i.e. `df_train`) - the output of the function that will be returned to the caller.

Good functions have a clear name and do one thing and it is very clear what they do. You feed something in and get something out. This is a key concept in software engineering: hiding complexity.
...
Looks like our pipeline is working. And here important building block for a production pipeline: A good logger! Often your code will run on different hardware somewhere, in the cloud, on a server or a machine from a colleague. If there is a problem, a log can give you vital information what is going wrong and how to fix it. You can find a saved version of this log in `logs/info.logs`.
...
Each pipeline has a clear input and output. Now the nice thing about Kedro is that you don't need to tell it which pipeline to run when. You connect them together by specifying inputs that are connected to outputs. For example the data_model pipeline takes the `train_encoded` dataset as input and returns a `model`, `X_test` and `y_test` lists. You can persist any dataset to storage by providing an entry for it in the catalog. So to keep the model, I added this entry to save it as pickle data:
...
Plus we can also consider some of the other software development best practices, such as automated testing to ensure the nodes and pipelines keep working the way we have designed them. We can also save this project easily to GitHub and share with colleagues. If we have kept the `requirements.txt` up to date, then they can easily run the same project and contribute. Or we can look at deploying our project to a cloud provider to build automatically under certain conditions, for example when new input data has arrived.
